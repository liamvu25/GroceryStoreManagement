<%-- 
    Document   : viewCart
    Created on : Mar 10, 2022, 9:10:18 AM
    Author     : Liamvu
--%>
            ...
            <c:if test="${not empty sessionScope.CART}">
                ...
                <c:if test="${sessionScope.CART.getCart().size() ne 0}">
                    ...
                    <form action="MainController">
                                ...
                                <c:forEach var="item" varStatus="counter" items="${sessionScope.CART.getCart()}">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <input type="hidden" name="uri" value="${item.value.productID}-${item.value.productName}-${item.value.price}-${item.value.category}"/>                                            
                        ...
                        <button style="background-color: #04AA6D" class="resbtn" name="action" value="UpdateCart" ><b>Update Cart</b></button>                                                
                    </form>
                    <form action="MainController" style="display: inline-block;color: #04AA6D">
                        <button style="background-color: #04AA6D" class="resbtn" name="action" value="ClearCart"><b>Clear Cart</b></button>  
                    </form>
                </c:if>
            </c:if>
        </c:if>
